...
## 📌 개요 (Overview)
본 기능은 사용자의 일상 시간(취침, 기상, 아침, 점심, 저녁 식사)에 맞춰 로봇(또는 디바이스)이 안부를 묻고 대화를 유도하는 **상호작용(문답) 기능**입니다.

> 로봇 **서비스 사용법 안내**(대화 호출, 복약 알림 등)는 `guide` 패키지이며 본 기능과 무관합니다. → [README_Service_Guide.md](README_Service_Guide.md) 사용자의 음성 응답을 인식하여 서버에 기록하고, 상황에 맞는 멘트와 모션을 통해 자연스러운 대화를 이어나갑니다.
